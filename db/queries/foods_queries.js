...
const addFoods = (foods) => {
  ...
  const query = `INSERT INTO foods (name, description, image_url, price) VALUES ($1, $2, $3, $4) RETURNING *;`;
...
const getQuantity = () => {
  const query = 'SELECT quantity FROM cart_items WHERE id = 1';

  db.query(query)
    .then(data => {
      console.log("quantity", data.rows[0].quantity);
